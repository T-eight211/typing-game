...
class TypingGame {
  constructor(apiUrl, displayElement, inputElement, mistakesElement, timerElement, tryAgainButton, wpmElement, database) {
  ...
  timer() {
    ...
    this._timer = setInterval(() => {
      ...
      if (this._timerElement.innerText == 0 || this.checkMatch()) {
        this.endTimer();
        setTimeout(() => {
          this._database.writeStats(this._renderer._displayElement, this._mistakesElement, this._timerElement, this._wpmElement);
        }, 1000);
      }
    }, 1000);
  }
...
class Database {
  constructor() {
    this._user_id = sessionStorage.getItem('userId');
    const today = new Date();
    this._formattedDate = parseInt(today.toISOString().slice(0, 10).replace(/-/g, ''));
  }

  writeStats(displayElement, mistakesElement, timerElement,wpmElement) {
    this._accuracy = Math.round(100-((mistakesElement.textContent/displayElement.innerText.length)*100));
    this._timeTaken = 60 - timerElement.innerText;
    const url = '/writeStats';
    const options = {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: this._user_id,
        date: this._formattedDate,
        wpm: wpmElement.innerText,
        accuracy: this._accuracy,
        timer_taken: this._timeTaken
      })
    };
    ...
    fetch(url, options)
      .then(response => {
        if (!response.ok) {
          throw new Error('Error adding game data to database!');
        }
...
window.onload = function() {
  ...
  // Store the userId value in sessionStorage
